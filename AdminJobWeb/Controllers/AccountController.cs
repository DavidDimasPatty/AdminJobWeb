...
namespace AdminJobWeb.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> CreatePassword([FromForm] admin dataObj)
        {
            ...
            try
            {
                tracelog.WriteLog($"User : {adminLogin}, Start Send Create Password for User {dataObj.username}");
                tracelog.WriteLog($"User : {adminLogin}, Start Hit Database Admin");
                ...
                <body>
                    ...
                    <br/>
                     <div>
                        <b>Link</b> : <a href='{linkSelf}/Account/CreateNewPassword?username={admin.username}&key={key}'>{linkSelf}/username={admin.username}&key={key}</a>
                    </div>
                    <br/>
                    <br/>
                    <div>
            ...
            {
                Debug.WriteLine(e);
                tracelog.WriteLog($"User : {adminLogin}, Failed Send Create Password, Reason : {e.Message}");
        ...
        public async Task<ActionResult> CreateNewPassword([FromForm] string username, string password, string passwordRet, string key)
        {
            if (password != passwordRet)
            {
                return Content($"<script>alert('Password Tidak Sama!');window.location.href='/Account/CreateNewPassword?username={username}&key={key}'</script>", "text/html");
            }
